...
**Output**: Space-separated integers of the valid permutation, or `"NO SOLUTION"`.  
...
## 3. Approach 1 — Naive (Backtracking / Next Permutation)

### Idea
Try all permutations using `std::next_permutation` until a beautiful one is found.
...
- **Optimality**: Output length is $n$, so $\Omega(n)$ is the theoretical minimum bound.
...
- **Fast I/O & Delimiters**: Outputting $10^6$ numbers requires Fast I/O (`cin.tie(NULL)`). Ensure space separation between elements and trailing newline `'\n'`.
...
2. **Q2: What if we need the lexicographically smallest beautiful permutation?**
   - **A**: The problem can be solved greedily or via backtracking with pruning for small $n$, or structured parity adjustments for larger $n$.
