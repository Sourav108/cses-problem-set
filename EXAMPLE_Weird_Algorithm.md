...
This is a worked benchmark note demonstrating the exact 10-section format required by [`AI_PROMPT_TEMPLATE.md`](./AI_PROMPT_TEMPLATE.md) for CSES problems.
...
int main() {
    // Standardized Fast I/O for competitive programming
...
### Complexity Derivation
- **Time Complexity**: $\mathcal{O}(K)$ transitions, printing $K + 1$ values (where $K \le 524$ for $n \le 10^6$). Total execution time is $< 1$ ms.
- **Space Complexity**: $\mathcal{O}(1)$ auxiliary memory — only a single 64-bit integer variable.
- **Optimality**: Matches the best known/asymptotically optimal complexity for the problem ($\Omega(K)$ lower bound to output $K+1$ values) and comfortably satisfies the CSES limits.

---

## 6. Correctness Proof

- **Deterministic Transition**: Let $f(n) = n / 2$ if $n \equiv 0 \pmod 2$, and $3n + 1$ if $n \equiv 1 \pmod 2$. The algorithm computes $n_{t+1} = f(n_t)$ sequentially from $n_0 = n$.
- **Loop Invariant**: At the start of step $t$, the output buffer contains the exact prefix sequence $\langle n_0, n_1, \dots, n_t \rangle$ generated by the Collatz recurrence.
- **Termination Guarantee**: The loop continues while $n > 1$ and terminates immediately upon reaching $n = 1$. For all $1 \le n \le 10^6$, computational verification establishes that the stopping time is finite ($K \le 524$). Thus, the loop terminates without infinite cycling.
- **Output Correctness**: The initial term $n_0$ is emitted before the loop, every intermediate value is output upon transition, and the sequence ends with $1$ followed by a newline, satisfying the problem specification.
